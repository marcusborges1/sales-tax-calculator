  ...
  describe '.determine_category' do
    context 'when product is a book' do
      it 'returns :book for "book"' do
        expect(described_class.determine_category('book')).to eq(:book)
      end
    ...
    context 'when product is food' do
      it 'returns :food for "chocolate bar"' do
        expect(described_class.determine_category('chocolate bar')).to eq(:food)
      end

      it 'returns :food for "box of chocolates"' do
        expect(described_class.determine_category('box of chocolates')).to eq(:food)
      end

      it 'returns :food for "candy"' do
        expect(described_class.determine_category('candy')).to eq(:food)
      end

      it 'returns :food for "cake"' do
        expect(described_class.determine_category('cake')).to eq(:food)
      end

      it 'returns :food for "imported box of chocolates"' do
        expect(described_class.determine_category('imported box of chocolates')).to eq(:food)
      end
    end

    context 'when product is medical' do
      it 'returns :medical for "headache pills"' do
        expect(described_class.determine_category('headache pills')).to eq(:medical)
      end

      it 'returns :medicine for "medicine"' do
        expect(described_class.determine_category('medicine')).to eq(:medical)
      end
    ...
    context 'when product is other' do
      it 'returns :other for "music CD"' do
        expect(described_class.determine_category('music CD')).to eq(:other)
      end

      it 'returns :other for "bottle of perfume"' do
        expect(described_class.determine_category('bottle of perfume')).to eq(:other)
      end

      it 'returns :other for unkown items' do
        expect(described_class.determine_category('counter strike 2')).to eq(:other)
      end
    end
  end
end
